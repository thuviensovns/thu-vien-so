...
export async function POST(req: NextRequest) {
  try {
    ...
    if (fileSize > MAX_FILE_SIZE) {
      return NextResponse.json({ error: `Video quá lớn. Tối đa ${MAX_FILE_SIZE / 1024 / 1024}MB` }, { status: 400 })
    }
    ...
    return NextResponse.json({ error: `Tạo URL upload video thất bại: ${msg}` }, { status: 500 })
  }
}
