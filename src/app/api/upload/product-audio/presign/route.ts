...
export async function POST(req: NextRequest) {
  try {
    ...
    if (fileSize > MAX_FILE_SIZE) {
      return NextResponse.json({ error: `Audio quá lớn. Tối đa ${MAX_FILE_SIZE / 1024 / 1024}MB` }, { status: 400 })
    }
    ...
    if (isR2Configured()) {
      ...
      console.log(`[Presign audio] Admin ${user.email} → ${r2Key} (${(fileSize / 1024 / 1024).toFixed(1)}MB, ${safeContentType})`)
    ...
    return NextResponse.json({ error: `Tạo URL upload audio thất bại: ${msg}` }, { status: 500 })
  }
}
